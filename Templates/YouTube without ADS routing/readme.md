...
Важно понимать, что метод не устраняет рекламу с ноды, на которой она изначально есть, он лишь пересылает подключение на ноду, где рекламы нет  
...
```json
    {
      ...
      "protocol": "vless",
      "settings": {
        "id": "User, имеющий доступ к этому inbound, можно специально создать в панели remnawave и привязать к соотвествующему Internal Squads", // https://docs.rw/learn/server-routing
